...
st.title("Project Wildfires")
st.sidebar.title("Sommaire")
pages=["Le projet Wildfire", "Exploration des données", "Visualisation", "Préparation des données", "Modélisation"]
...
if page == pages[2]:
    ...
    fires[['DISC_TIME', 'DISC_HOUR', 'DISC_MIN', 'CONT_TIME', 'CONT_HOUR', 'CONT_MIN']].info(verbose=True,
                                                                                             memory_usage=True,
                                                                                             show_counts=True)
    ...
    fires['CAUSE_DESCR_HUMAN'] = fires['CAUSE_DESCR'].apply(
        lambda x: 1 if x not in ['Lightning', 'Missing/Undefined'] else 0)
    ...
    dict_states = {'AK': 'Alaska', 'AL': 'Alabama', 'AZ': 'Arizona', 'AR': 'Arkansas', 'CA': 'Californie',
                   'CO': 'Colorado',
                   'CT': 'Connecticut', 'DE': 'Delaware', 'FL': 'Floride', 'GA': 'Géorgie', 'HI': 'Hawaï',
                   'ID': 'Idaho',
                   ...
                   'VA': 'Virginie',
                   'WA': 'Washington', 'WV': 'Virginie-Occidentale', 'WI': 'Wisconsin', 'WY': 'Wyoming',
    ...
    fires_veg_duplicated = fires_veg.loc[fires_veg['is_id_duplicated'] == True, ['FPA_ID']].sort_values(by='FPA_ID')[
        'FPA_ID'].values
    ...
    ratio_null_values_NBCD = fires_veg.loc[fires_veg[
                                               'NBCD_countrywide_biomass_mosaic'] == 0, 'NBCD_countrywide_biomass_mosaic'].count() / \
                             fires_veg.shape[0]
    ...
    print(
        f"On constate qu'il y a {len(fires_fpa_set - fires_veg_fpa_set)} 'FPA_ID' uniques dans le dataset Kaggle qui ne sont pas présentes dans le dataset complémentaire.")
    print(
        f"On constate qu'il y a {len(fires_veg_fpa_set - fires_fpa_set)} 'FPA_ID' uniques dans le dataset complémentaire qui ne sont pas présentes dans le dataset Kaggle.")
    ...
    fire_size_cumul_hum = \
        fires.loc[fires['CAUSE_DESCR_HUMAN'] == 1].groupby(['STATE', 'DISC_YEAR', 'CLASS', 'CAUSE_DESCR'],
                                                           observed=False)[
            'SIZE'].sum().reset_index()
    fire_size_cumul_lig = \
        fires.loc[fires['CAUSE_DESCR'] == 'Lightning'].groupby(['STATE', 'DISC_YEAR', 'CLASS', 'CAUSE_DESCR'],
                                                               observed=False)[
            'SIZE'].sum().reset_index()
    ...
    fires_count = pd.DataFrame(
        fires.groupby(['STATE', 'DISC_YEAR'], observed=False)['FOD_ID'].count().rename('COUNT')).reset_index()
    fires_count.head()
    ...
    map_nb_feu = px.scatter_geo(fires_count, locations="STATE", size='COUNT', size_max=35,
                                color='COUNT',
                                color_continuous_scale=['rgb(254,217,118)', 'rgb(254,178,76)', 'rgb(253,141,60)',
                                                        'rgb(252,78,42)',
                                ...
                                labels={'STATE': 'Code Etat', "STATE_NAME": "Nom de l'état", 'COUNT': 'Nombre',
                                        'DISC_YEAR': 'Année'},
                                hover_data=["STATE_NAME", 'DISC_YEAR', 'COUNT'],
                                height=700, width=1200
                                )
    map_nb_feu.update_traces(
        hovertemplate='Etat : %{customdata[0]}<br>Année : %{customdata[1]}<br>Nombre : %{customdata[2]:.3s}')
    ...
    fires_size = pd.DataFrame(
        fires.groupby(['STATE', 'DISC_YEAR'], observed=False)['SIZE'].sum().rename('CUMUL_SIZE')).reset_index()
    fires_size.head()
    ...
    map_surf_cum = px.scatter_geo(fires_size, locations="STATE", size='CUMUL_SIZE', size_max=50,
                                  ...
                                  color_continuous_scale=['rgb(254,217,118)', 'rgb(254,178,76)', 'rgb(253,141,60)',
                                                          'rgb(252,78,42)',
                                                          'rgb(227,26,28)', 'rgb(189,0,38)', 'rgb(128,0,38)'],
                                  labels={'STATE': 'Code Etat', "STATE_NAME": "Nom de l'état",
                                          'CUMUL_SIZE': 'Surface cumulée', 'DISC_YEAR': 'Année'},
    ...
    map_surf_cum.update_traces(
        hovertemplate='Etat : %{customdata[0]}<br>Année : %{customdata[1]}<br>Surface cumulée : %{customdata[2]:.3s} acres')
    ...
    data = {'Cause': ['Foudre', 'Facteurs humains', 'Missing/Undefined'],
            "Nombre d'incendies": [natural_sum, human_sum, missing_sum]}
    ...
    fire_size_cumul = fires.groupby('CAUSE_DESCR', observed=False)['SIZE'].sum().sort_values(
    ...
    fires_count = fires_count.groupby('CAUSE_DESCR', observed=False)['CAUSE_DESCR'].count().sort_values(
        ascending=False).rename('COUNT')
    fires_size_cumul = fires[['SIZE', 'CAUSE_CODE', 'CAUSE_DESCR']]
    fires_size_cumul = fires_size_cumul.groupby('CAUSE_DESCR', observed=False)['SIZE'].sum().sort_values(
        ascending=False).rename('SIZE_CUMUL')
    ...
    nb_vs_surf_cum = go.Figure(
        ...
        layout={
            ...
            'yaxis2': {'title': 'Nombre de feux', 'overlaying': 'y', 'side': 'right', 'showgrid': False}
    ...
    # Create a pie chart
    pie_surfcum_cause = go.Figure(
        data=[go.Pie(labels=labels, values=fire_s_cumul['SIZE'], pull=[0.2, 0, 0.2, 0.4, 0, 0, 0, 0, 0, 0, 0, 0, 0],
    ...
    nb_state_cause = px.histogram(fires, x="STATE", color="CAUSE_DESCR",
                                  color_discrete_map=dict_causes_colors,
                                  labels={'CAUSE_DESCR': 'Cause', 'STATE': 'State'}).update_xaxes(
    ...
    fires_size_cumul = fires_size_cumul.groupby('DISC_YEAR')['SIZE'].sum().sort_values(ascending=False).rename(
        'SIZE_CUMUL')
    ...
    fires_saison_hum = fires.loc[fires['CAUSE_DESCR'] != 'Lightning'].groupby(['DISC_YEAR', 'DISC_MONTH'])[
        'FOD_ID'].count().rename('COUNT')
    ...
    hmap_nb_human.update_xaxes(tickangle=0, tickfont=dict(family="verdana", color='black', size=12), dtick=2,
                               title_text=None)
    ...
    fires_saison_nat = fires.loc[fires['CAUSE_DESCR'] == 'Lightning'].groupby(['DISC_YEAR', 'DISC_MONTH'])[
        'FOD_ID'].count().rename('COUNT')
    ...
    hmap_nb_natural.update_xaxes(tickangle=0, tickfont=dict(family="verdana", color='black', size=12), dtick=2,
                                 title_text=None)
    ...
    veg_moist_class = px.box(fires_veg_moisture, x='CLASS', y='FUEL_MOISTURE', color='CLASS',
                             category_orders={"CLASS": ["A", "B", "C", "D", "E", "F", "G"]},
                             labels={'FUEL_MOISTURE': "Taux d'humidité du combustible végétal",
                                     'CLASS': 'Classe de feu'}
                             )
    ...
    #st.plotly_chart(  # fig)
    #st.plotly_chart(  # fig)
#if st.checkbox("Afficher la carte du nombre de feu par Etat et par année", value=True):
    st.plotly_chart(map_nb_feu)
#if st.checkbox("Afficher la carte des surfaces cumulées par Etat et par année", value=True):
    st.plotly_chart(map_surf_cum)
#if st.checkbox("Afficher la carte de densité de l'évolution temporelle des feux de classe E", value=True):
    st.plotly_chart(density_map)
#if st.checkbox("Afficher la distribution des classes de feux par an", value=True):
    st.plotly_chart(nb_class_year)
#if st.checkbox("Afficher la proportion des 3 catégories de causes", value=True):
    st.plotly_chart(cause_3_feu)
    #st.plotly_chart(  # fig)
#if st.checkbox("Afficher la distribution des surfaces cumulées par cause humaines vs naturelles", value=True):
    st.plotly_chart(surf_cum_cause)
#if st.checkbox("Afficher la distribution du nombre vs surfaces cumulées par cause", value=True):
    st.plotly_chart(nb_vs_surf_cum)
#if st.checkbox("Afficher la proportion des causes pour la surface cumulée", value=True):
    st.plotly_chart(pie_surfcum_cause)
#if st.checkbox("Afficher la distribution du nombre de feux des causes par Etats", value=True):
    st.plotly_chart(nb_state_cause)
#if st.checkbox("Afficher la distribution des sufaces cumulées des causes par Etats", value=True):
    st.plotly_chart(surfcum_state_cause)
    #st.plotly_chart(  # plt)
    #st.plotly_chart(  # plt)
    #st.plotly_chart(  # fig)
#if st.checkbox("Afficher la heatmap de la saisonnalité des causes humaines", value=True):
    st.plotly_chart(hmap_nb_human)
    #st.plotly_chart(fig)
#if st.checkbox("Afficher la heatmap de la saisonnalité des causes naturelles", value=True):
    st.plotly_chart(day_human_natural)
#if st.checkbox("Afficher le boxplot de la variable 'wind' par classe", value=True):
    st.plotly_chart(wind_class)
#if st.checkbox("Afficher le boxplot de la variable 'moisture' par classe", value=True):
    st.plotly_chart(veg_moist_class)
    #st.plotly_chart(  # biomass_class)
#if st.checkbox("Afficher la distribution des types de végétation par classe", value=True):
    st.plotly_chart(veg_class)
